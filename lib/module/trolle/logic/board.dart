...
class BoardState {
  ...
  final List<Board> myData;
  final List<Board> ortherData;
  final String? error;
...
class BoardNotifier extends Notifier<BoardState> {
  ...
  BoardState build() {
    ...
    return BoardState(isLoading: false, data: [], error: null, myData: [], ortherData: []);
  }
  ...
  Future<bool> getList() async {
    ...
    try {
      ...
      if (response.statusCode == 200 && response.data['code'] == 200) {
        ...
        // print(data);
        final listData = data.map((i) => Board.fromJson(i)).toList();
  ...
  Future<bool> deleteRequest(dynamic data) async {
    try {
      ...
      final dio = Dio();
      dio.options.headers['Authorization'] = 'Bearer $token';
      String url = '$baseUrl/board/delete';
...
final BoardProvider = NotifierProvider<BoardNotifier, BoardState>(BoardNotifier.new);

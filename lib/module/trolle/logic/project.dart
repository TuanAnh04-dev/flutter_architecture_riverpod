...
class ProjectState {
  ...
  final List<Project> myData;
  final List<Project> ortherData;
  final String? error = null;

  ProjectState({required this.isLoading, required this.data, required this.myData, required this.ortherData, error});
...
class ProjectNotifier extends Notifier<ProjectState> {
  ...
  ProjectState build() {
    ...
    return ProjectState(isLoading: false, data: [], error: null, myData: [], ortherData: []);
  }
  ...
  Future<bool> getList() async {
    ...
    try {
      ...
      if (response.statusCode == 200 && response.data['code'] == 200) {
        ...
        // print(data);
        final listData = data.map((i) => Project.fromJson(i)).toList();
  ...
  Future<bool> deleteRequest(dynamic data) async {
    try {
      ...
      final dio = Dio();
      dio.options.headers['Authorization'] = 'Bearer $token';
      String url = '$baseUrl/project/delete';
